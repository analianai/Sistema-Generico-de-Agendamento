...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['acao']) && $_POST['acao'] === 'excluir') {
        ...
        if (!$stmt) {
            $_SESSION['mensagem_erro'] = 'Erro interno ao tentar excluir o agendamento.';
        } else {
            $stmt->bind_param("i", $agendamento_id);
            if ($stmt->execute()) {
                $_SESSION['mensagem_sucesso'] = 'Agendamento excluído com sucesso!';
            } else {
                $_SESSION['mensagem_erro'] = 'Erro ao excluir o agendamento: ' . $stmt->error;
            }
            $stmt->close();
        }
        header("Location: user_agendamento.php");
        exit;
    } else {
        // **Lógica de cadastro**
        ...
        if (!$stmt) {
            $_SESSION['mensagem_erro'] = 'Erro interno ao tentar realizar o agendamento.';
        } else {
                    ...
                    <tbody>
                        <?php while ($agendamento = $resultAgendamentos->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td class="text-center"><?php echo $agendamento['categoria']; ?></td>
                                <td class="text-center"><?php echo date('d/m/Y H:i', strtotime($agendamento['data_hora'])); ?></td>
                                <td class="text-center"><?php echo $agendamento['duracao']; ?> minutos</td>
                                <td class="text-center"><?php echo $agendamento['status']; ?></td>
